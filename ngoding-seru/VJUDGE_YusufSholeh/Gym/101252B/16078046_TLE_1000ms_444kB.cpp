  ...
  for(int i = 9; i >= 1; i--) {
    ...
    if (sisa[line1][i] && sisa[line2][i] && s1 - i >= 0 && s2 - i >= 0) {
      if (ukur[line1] == 1 && s1 - i != 0) {} 
        ...
        int sis_sum = 0;
        int sis2_sum = 0;
        for(int j = 1; j <= 9; j++) sis_sum += (sisa[line1][j] ? j : 0);
        for(int j = 1; j <= 9; j++) sis2_sum += (sisa[line2][j] ? j : 0);

        if (sis_sum - i < sum[line1] - i) continue;
        if (sis2_sum - i < sum[line2] - i) continue;
