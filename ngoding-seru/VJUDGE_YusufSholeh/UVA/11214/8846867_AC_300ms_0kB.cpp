...
void dfs(int x, int y, int step) {
	...
	rc[x][y] = true;
	dfs(x, y + 1, step + 1);
	if (ad == false) diag1[x-y+off] = false;
	if (ar == false) row[x] = false; 
	if (ac == false) col[y] = false;
	if (ad2 == false) diag2[x+y] = false;
	if (arc == false) rc[x][y] = false;
	dfs(x, y + 1, step);
}
