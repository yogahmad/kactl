...
int main() {
	...
	while(scanf("%d %d",&n, &m) != EOF) {
		...
		while(!pq.empty()) {
			...
			if (flag[x][y][top.f.s] == cs) continue;
			flag[x][y][top.f.s] = cs;
			
			go(top.f.f, top.f.s, kanan[x][y]);
			go(top.f.f, top.f.s, kiri[x][y]);
			go(top.f.f, top.f.s, atas[x][y]);
			go(top.f.f, top.f.s, bawah[x][y]);
		}
