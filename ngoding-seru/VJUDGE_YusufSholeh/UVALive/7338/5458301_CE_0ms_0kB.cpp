...
const int root = 1;
int tc, n, m, ptr, cnt, ans;
int P[MAX][16];
int L[MAX], sub[MAX], arr[MAX];
int lg[MAX];
int ChainHead[MAX], index[MAX], ChainNo[MAX];
int Tmax[4*MAX];
...
inline void clear(int len) {
	...
	for(int i = 0; i < len; i++) {
		...
		PPTR[i] = -1;
		L[i] = ChainHead[i] = -1;
		index[i] = arr[i] = ChainNo[i] = sub[i] = 0;
...
void update(int node, int l, int r, int a, int b, int v) {
	...
	// Tmin[node] = (Tmin[node<<1] < Tmin[node<<1|1]) ? Tmin[node<<1] : Tmin[node<<1|1];
}
...
inline int query_up(int u, int v, int w) {
	...
	while(true) {
		if (uchain == vchain) {
			if (u != v) {
				int res = query_tree(1, 1, ptr - 1, index[v] + 1, index[u]);
				update(1, 1, ptr - 1, index[v] + 1, index[u], w);
				if (res > ret) ret = res;
			}
			break;
		}
		int res = query_tree(1, 1, ptr - 1, index[ChainHead[uchain]], index[u]);
		update(1, 1, ptr - 1, index[ChainHead[uchain]], index[u], w);
		if (res > ret) ret = res;
		u = ChainHead[uchain];
		u = P[u][0];
		uchain = ChainNo[u];
...
int main() {
	...
	for(int cs = 1; cs <= tc; cs++) {
		...
		for(int i = n; i <= m; i++) {
			...
			// printf("%d %d %d %d\n",E[i].s.f, E[i].s.s, aa, bb);
			
			ans += -i + i*i*(E[i].f - ((aa < bb) ? bb : aa));
		}
