...
struct inner{
	...
	void delete_point(point p) {
		multiset<point>::iterator it = mset.lower_bound(p);
		mset.erase(it);
	}
	
} segtree[TMAX];

void Delete(LL now, LL l, LL r, point p) {
	segtree[now].delete_point(p);
	if (l == r) return;
	
	LL mid = (l + r) >> 1;
	if (p.x <= mid) Delete(now << 1, l, mid, p);
	else			Delete(now << 1 | 1, mid + 1, r, p);
}
...
int main() {
	...
	for(LL cs = 1; cs <= tc; cs++) {
		...
		for(LL i = 1; i <= Q; i++) {
			LL qx, qy;
			scanf("%lld %lld",&qx, &qy);
			scanf("%lld %lld %lld %lld %lld %lld %lld",&Ek, &a, &b, &c, &d, &e, &f);
