...
int main() {
	...
	int n = a.length();
	for(int i = 0; i < n; i++) {
		ans1 += (long long)n - A.SA[i];
		if (i != n-1) {
			ans1 -= (long long)A.compute_lcp(A.SA[i], A.SA[i+1]);
		}
	}
	...
	int m = b.length();
	for(int i = 0; i < m; i++) {
		ans2 += (long long)m - B.SA[i];
		if (i != m-1) {
			ans2 -= (long long)B.compute_lcp(B.SA[i], B.SA[i+1]);
		}
	}
	...
	long long ans3 = 0;
	for(int i = 0; i < o; i++) {
		ans3 += (long long)o - C.SA[i];
		if (i != o-1) {
			ans3 -= (long long)C.compute_lcp(C.SA[i], C.SA[i+1]);
		}
	}
	
	//cout << ans3 << endl;
	ans3 -= ((long long)(n+1) * (long long)(m+1));
