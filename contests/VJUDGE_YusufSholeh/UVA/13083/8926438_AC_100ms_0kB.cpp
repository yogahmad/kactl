...
long long factorize(LL N) {
	...
	while(!q.empty()) {
		...
		int iter = 0;
		while(d == 1) {
			// if (maps.count(x)) printf("WFWRFFRWFWRFRFWFWRFWFAFJSIFSAJOIFJASIOFJ %d\n",maps[x]);
			step++;
			// maps[x] = step;
			x = g(x, num);
			LL k = x - y;
	...
	// print(result);
	map<LL,int> maps;
	for(int i = 0; i < result.size(); i++) maps[result[i]] += 1;
	
	LL ans = 1;
	LL sum = 0;
	LL kali = 1;
	for(map<LL,int>::iterator it = maps.begin(); it != maps.end(); it++) {
		LL angka = (*it).first;
		LL ada = (*it).second;
		LL ampz = 0;
		LL loopz = 1;
		for(LL i = 0; i <= ada; i++) {
			ampz += ((ada - i)*2 + 1)*loopz;
			// printf("%lld\n",ampz);
			loopz *= angka;
		}
		ans *= ampz;
