...
void update(int now, int l, int r, int a, int b, int val, int valC) {
	// if (now == 1) {
		// printf("%d %d %d\n",a, b, val);
	// }
	if (done[now]) return;
	if (r < a || b < l) return;
	if (a <= l && r <= b) {
		if (val == -1) done[now] = true, tree[now] = min(tree[now], valC);
		else tree[now] = min(tree[now], val);
		return;
	}
	valC = min(valC, tree[now]);
	int mid = (l + r) >> 1;
	update(now * 2, l, mid, a, b, val, valC);
	update(now * 2 + 1, mid + 1, r, a, b, val, valC);
}
...
int main() {
	...
	for(int i = 0; i < n; i++) {
		...
		update(1, 1, n, 1, length, realIndex, INF);
		int lcpNext = (i < n) ? A.computeLCP(A.SA[i], A.SA[i+1]) : 0;
		update(1, 1, n, lcpNext+1, length, -1, INF);
	}
