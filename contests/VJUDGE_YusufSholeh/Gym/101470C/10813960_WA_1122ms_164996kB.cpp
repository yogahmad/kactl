...
void print_grid(vector< vector<int> > W) {
    for(int i = 0; i < r; i++) {
        for(int j = 0; j < c; j++) {
            printf("%d ",W[i][j]);
        }
...
int main() {
    ...
    for(int i = 0; i < r; i++) {
        for(int j = 0; j < c; j++) {
            int i_min_1 = (i == 0) ? 0 : W[i-1][j];
            int j_min_1 = (j == 0) ? 0 : W[i][j-1];
            int i_min_1_j_min_1 = (i > 0 && j > 0) ? W[i-1][j-1] : 0;
            W[i][j] += i_min_1 + j_min_1 - i_min_1_j_min_1;

            if (i-P+1 >= 0 && j-P+1 >= 0) {
                int i_min_p = (i-P >= 0) ? W[i-P][j] : 0;
                int j_min_p = (j-P >= 0) ? W[i][j-P] : 0;
                int i_min_p_j_min_p = (i-P >= 0 && j-P >= 0) ? W[i-P][j-P] : 0;
                int sum = W[i][j] - i_min_p - j_min_p + i_min_p_j_min_p;
